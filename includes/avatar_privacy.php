...
function hc_custom_bp_core_fetch_avatar( $avatar_html, $params ) {
	...
	$url = \apply_filters( 'avatar_privacy_default_icon_url', \includes_url( 'images/blank.gif' ), $hash, $params['width'], [
			'default' => 'identicon',
	] );
	...
	if ( $show_gravatar ) {
		...
		$url = \apply_filters( 'avatar_privacy_gravatar_icon_url', $url, $hash, $params['width'], [
			'user_id'  => $user_id,
			'email'    => $email,
			'rating'   => 'g',
			'mimetype' => $mimetype,
 ...
 *
 *  @param string       	 $avatar       Display height of the avatar in pixels. Defaults to $size.
...
function hc_custom_get_avatar( $avatar, $id_or_email, $size, $default, $alt ) {
	...
	if ( ! empty( $user_id ) ) {
		$avatar = bp_core_fetch_avatar( array( 'item_id' => $user_id, 'height' => $size, 'width' => $size, 'default' => 'identicon' ) );
	}
